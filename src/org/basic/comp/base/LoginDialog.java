...
public class LoginDialog {
	...
	protected void aksiOk() {
		...
		try {
			...
			ODocument usr=App.getUsrDao().getOne(db, Usr.USERNAME, u);
			if (usr==null) {
				App.showErrorUsernameTidakTerdaftar();
			}else{
				UtilAccount util=new UtilAccount();
				String tmp=util.md5(p);
				String pwd=usr.field(Usr.PASSWORD);
				if (tmp.equalsIgnoreCase(pwd)) {
					DataUser.setUsr(usr);
					ODocument grp = App.getGrpDao().getOne(db, "@rid", DataUser.getUsr().field(Usr.GRP));
					DataUser.setGrp(grp);
